...
// jsDynamicPatterns are per-language patterns for JavaScript and TypeScript.
// Registered via init() into dynamicPatternsByLang.
//
// NOTE: This file is named dynamic_patterns_javascript.go (not _js.go).
// Go silently excludes *_js.go files as GOOS=js build-constrained — using
// the full "javascript" name avoids that trap.

...
// TestDynamicPatterns_Python covers the pythonDynamicPatterns catalog.
// Each row asserts that a representative stub is classified as Dynamic under
// the Python language gate, and that obvious cross-language collisions are NOT
// classified as dynamic.
func TestDynamicPatterns_Python(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		// Per-language gate: click DSL names from non-Python files MUST NOT classify.
		...
		// Cross-lang: JVM bare names MUST NOT fire for Python.

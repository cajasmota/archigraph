...
// TestDynamicPatterns_Dart covers the dartDynamicPatterns catalog.
func TestDynamicPatterns_Dart(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		// Note: `setState` for JavaScript fires via set[A-Z] React pattern, so skip JS gate test.
		...
		// Widget names that are unique to Dart (not in Python Flask catalog).

...
| flask-realworld | python | 43 | 5.85% (2026-05-19, python-django-w5; was 7.60% w4) | python-django-w5 | EXTENDS kind-disambiguator (lookupLocationKind + lookupByKindHint two-tier scan: real entity kinds beat SCOPE.* shadows; nameKinds no longer dual-indexes SCOPE→trimmed), cross-file Format A fallback (lookupGlobalByFamily), DEPENDS_ON/IMPORTS hint-family + kind-mismatch fallback, dotted ClassName.save/.delete/.to_json class-attribute classifier, Python builtin exceptions in pythonExternalBaseTypes, Route:* URLConf→Dynamic, `first` claimed by pythonBareNames. Residual: `cls`/`walk`/`remove`/`post`/`pop`/`offset`/`limit`/`exit`/`append` generic verbs (collision-prone), `now`/`utcnow` bare instance helpers | at-bar | one more wave for ≤1% (generic-verb allowlist with receiver-type gate) |

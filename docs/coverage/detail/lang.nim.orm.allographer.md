...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Query attribution | ✅ `full` | `2026-06-14` | 5116 | `internal/custom/nim/allographer_query.go`<br>`internal/custom/nim/extractors_test.go` | The Allographer rdb() query builder (`rdb().table("t")...<op>()`) is attributed to its table by the query extractor (custom_nim_allographer_query, pre-filtered by nimAllographerHasQuery so schema-only files and arbitrary Nim are ignored; now also admits `.raw(` chains). Each `rdb()` head bounds one query chain; the `.table("...")` anchor gives the table identity (the same key the schema-builder table uses, so query->table converges by name) and the terminal builder method classifies the op: `.get()/.first()/.find(/.pluck(/.count(/.max(/.min(/.avg(/.sum(` -> select; `.insert(/.insertId(/.insertID(` -> insert; `.update(` -> update; `.delete(` -> delete. One SCOPE.Schema/table per distinct table carries a QUERIES edge table->table (bare table name, resolved by the shared resolver) per distinct operation (operation + table props), reusing the SCOPE.Schema Kind + QUERIES edge the Norm/Debby query-attribution extractors use (no new kind). #5116 deepening: (a) JOIN targets (`.join/.leftJoin/.rightJoin/.innerJoin/.outerJoin/.crossJoin("other",...)`) yield an additional QUERIES edge (operation=select, join=true) per joined table, so a multi-table read converges on every accessed table; (b) raw SQL via `rdb().raw("...")` is best-effort parsed for its FROM/JOIN/INTO/UPDATE target table(s) (raw=true, op classified from the leading SQL verb) even with no `.table()` anchor; (c) a `.table(ident)` whose argument is a const/let/var string-literal binding is resolved to its literal (shared nimAlloStrBindingRe), while a truly unbound identifier is skipped (no fabricated query). Proven by TestNimAllographerQuery_AttributesOps (users select+insert, posts update+delete) + TestNimAllographerQuery_JoinsRawDynamicTxn (join=true, raw=true FROM/JOIN/INTO targets, const-bound dynamic-table resolution + unbound skip) + TestNimAllographerQuery_RawNoMatchNoop + TestNimAllographerQuery_NonQueryNoop + TestNimAllographerQuery_WrongLanguageNoop. Honest remainder: raw-SQL parsing is a FROM/JOIN/INTO/UPDATE table scan, not a full SQL parse (CTEs/subqueries not decomposed). |
...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Transaction function stamping | ✅ `full` | `2026-06-14` | 5116 | `internal/custom/nim/allographer_query.go`<br>`internal/custom/nim/extractors_test.go` | Allographer transactions are run via the rdb() query-builder transaction API (`rdb().transaction(proc() = ...)`). The query extractor bounds each `rdb().transaction(...)` block with a balanced-paren scan (txnBlockEnd) and (i) stamps transaction=true on the QUERIES edge of every rdb() query whose chain head falls inside that block, and (ii) #5116(d): synthesises a standalone SCOPE.Operation/transaction boundary entity per block (Name=rdb().transaction, framework=allographer, provenance=INFERRED_FROM_ALLOGRAPHER_TRANSACTION, transaction=true), reusing the SCOPE.Operation Kind (no new kind). Proven by TestNimAllographerQuery_AttributesOps (accounts.update + ledger.insert inside an rdb().transaction(...) asserted transaction=true) + TestNimAllographerQuery_JoinsRawDynamicTxn (standalone transaction boundary entity asserted + enclosed-query stamp preserved). Honest remainder: nested/named transactions are not distinguished (each rdb().transaction(...) head is one boundary entity). |

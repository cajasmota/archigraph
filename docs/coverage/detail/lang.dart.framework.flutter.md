...
| Prop extraction | ✅ `full` | `2026-06-24` | 5361 | `internal/custom/dart/flutter_props_test.go` | #5361: widget prop / tree depth. Each widget class (Stateless/Stateful/Consumer/Hook) has its final <Type> <name>; instance fields (the canonical Flutter prop idiom, bound by required this.<name> ctor params) extracted as SCOPE.Pattern(prop_extraction) entities (prop_name/prop_type/widget) plus a HAS_PROPS edge from the widget component (reFlutterWidgetProp; scoped to widgetNames so model/service fields are not mis-tagged). Function-typed and nullable props handled (void Function()?). Proven by TestFlutterWidgetProps (UserCard title:String,count:int,onTap -> 3 HAS_PROPS; non-widget field 'secret' excluded). |
